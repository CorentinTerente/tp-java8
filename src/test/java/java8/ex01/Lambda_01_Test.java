...
public class Lambda_01_Test {
    ...
    public void test_filter_by_age() throws Exception {
        ...
        List<Person> result = personList.stream().filter(p -> p.getAge()>=18).collect(Collectors.toList());
    ...
    public void test_filter_by_firstname() throws Exception {
        ...
        List<Person> result = personList.stream().filter(p -> p.getFirstname().equals("first_10")).collect(Collectors.toList());
    ...
    public void test_filter_by_password() throws Exception {
        ...
        List<Person> result = personList.stream().filter(p -> p.getAge()>49 && DigestUtils.sha512Hex(p.getPassword()).equals(passwordSha512Hex)).collect(Collectors.toList());
